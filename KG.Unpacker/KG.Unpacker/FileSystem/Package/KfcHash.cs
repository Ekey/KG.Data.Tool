...
namespace KG.Unpacker
{
    class KfcHash
    {
        private static readonly UInt64[] crc64_tab = {
        ...
        public static UInt64 iGetHash(String lpString)
        {
            UInt64 dwHash = 0xFFFFFFFFFFFFFFFF;
            for (Int32 i = 0; i < lpString.Length; i++)
            {
                dwHash = (dwHash >> 8) ^ crc64_tab[(Byte)(dwHash ^ lpString[i])];
            }
